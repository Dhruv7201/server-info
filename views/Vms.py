...
vms = Blueprint('vms', __name__, template_folder='templates/vms', static_folder='static', url_prefix='/vm')
...
def insert_vm_form():
    ...
    available_keys.sort()
    available_values = VmConfig.query.with_entities(VmConfig.meta_value).distinct().all()
    available_values = [value[0] for value in available_values]
    available_values.sort()

    return render_template('insert_vm_form.html', available_keys=available_keys, available_values=available_values, server_id=server_id)
...
def update_vm_form():
    ...
    available_values = [value[0] for value in available_values]
    available_values.sort()
    return render_template('update_vm_form.html', existing_vm_configs=existing_vm_configs, available_keys=available_keys, available_values=available_values, server_id=server_id)
