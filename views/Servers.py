...
servers = Blueprint(
    "servers",
    __name__,
    template_folder="templates/servers",
...
def index():
    ...
    if user.superuser:
        ...
        if data_center_params:
            ...
            servers = user.servers.filter(
                Server.data_center_id.in_(data_center_params)
            ).all()
        else:
...
def insert_server_form():
    ...
    available_keys.sort()
    available_values = (
        Config.query.with_entities(Config.meta_value)
        .distinct()
        .filter(Config.meta_key != "password")
        .filter(Config.meta_key != "idrac_password")
        .all()
    )
...
def update_server_form(server_id):
    ...
    if request.method == "GET":
        ...
        available_keys.sort()
        available_values = (
            Config.query.with_entities(Config.meta_value).distinct().all()
        )
        available_values = [value[0] for value in available_values]
        available_values.sort()
        return render_template(
            "update-server-form.html",
            server=server,
            datacenters=datacenters,
            available_keys=available_keys,
            available_values=available_values,
    ...
    if request.method == "POST":
        ...
        if server:
            ...
            for key, value in zip(new_config_keys, new_config_values):
                if key and value:
                    new_config = Config(
                        meta_key=key, meta_value=value, server_id=server.id
                    )
...
def delete_server(server_id):
    ...
    db.session.execute(user_server.delete().where(user_server.c.server_id == server_id))
