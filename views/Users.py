...
users = Blueprint('users', __name__, template_folder='templates/users', static_folder='static', url_prefix='/user')
...
def add_user_form():
    ...
    for data_center in data_centers:
        ...
        data_centers_servers[data_center.id]['data_center'] = data_center.serialize()  # Serialize DataCenter
        data_centers_servers[data_center.id]['servers'] = [server.serialize() for server in Server.query.filter_by(data_center_id=data_center.id).all()]

    users = User.query.all()

    return render_template('add_user.html', data_centers_servers=data_centers_servers, users=users)
...
def add_user():
    if request.method == 'POST':
        ...
        new_user = User(username=user_id, password=password_hash, superuser=superuser, role='user')
...
def edit_user_form(user_id):
    ...
    for data_center in data_centers:
        ...
        data_centers_servers[data_center.id]['servers'] = Server.query.filter_by(data_center_id=data_center.id).all()
    for data_center_id in data_centers_servers:
        for server in data_centers_servers[data_center_id]['servers']:
            server.serialize()
    return render_template('edit_user.html', user=user, data_centers_servers=data_centers_servers)
