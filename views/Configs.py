...
configs = Blueprint(
    "configs",
    __name__,
    template_folder="templates/configs",
...
def index():
    ...
    configs = (
        Config.query.filter(Config.server_id.in_([server.id for server in servers]))
        .order_by(Config.meta_key)
        .all()
    )
    vm_configs = (
        VmConfig.query.filter(VmConfig.server_id.in_([server.id for server in servers]))
        .order_by(VmConfig.meta_key)
        .all()
    )
    ...
    for key in key_list:
        ...
        filters[key] = filters[key] = sorted(
            {config.meta_value for config in configs if config.meta_key == key}
        )

        filters[key].extend(
            sorted(
                {config.meta_value for config in vm_configs if config.meta_key == key}
            )
        )
        # remove duplicates
...
def filter():
    ...
    if "last_updated_at" in selected_values:
        if selected_values["last_updated_at"] != "None":
            ...
            servers = servers.filter(
                Server.updated_at > datetime.now() - timedelta(hours=hours)
            )
    ...
    if "datacenter" in selected_values:
        datacenter_ids = [
            DataCenter.query.filter_by(name=datacenter).first().id
            for datacenter in selected_values["datacenter"]
        ]
    ...
    for meta_key, meta_values in selected_values.items():
        if "datacenter" != meta_key:
            ...
            servers = servers.outerjoin(
                config_alias, Server.id == config_alias.server_id
            ...
            query_filters.append(
                or_(
                    and_(
                        config_alias.meta_key == meta_key,
                        config_alias.meta_value.in_(meta_values),
                    ),
                    and_(
                        vm_config_alias.meta_key == meta_key,
                        vm_config_alias.meta_value.in_(meta_values),
                    ),
                )
            )
